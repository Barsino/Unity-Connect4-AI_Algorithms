...
public abstract class Algorithm : ScriptableObject
{
    ...
    protected int Evaluate(int[,] board, int player)
    {
        ...
        {
            if (board[centerColumn, row] == player)
                score += 100;
            else if (board[centerColumn, row] == opponent)
                score -= 100;
        }
                    ...
                    score += EvaluateWindow(new int[]
{
    ...
    private int EvaluateWindow(int[] window, int player, int opponent)
    {
        ...
        if (playerCount == 4) { score += 1000; }// Ganar

        else if (playerCount == 3 && emptyCount == 1) { score += 100; }// Tres en línea con un espacio

        else if (playerCount == 2 && emptyCount == 2) { score += 30; }// Dos en línea con dos espacios

        if (opponentCount == 4) { score -= 1000; }// Oponente gana

        else if (opponentCount == 3 && emptyCount == 1) { score -= 150; }// Oponente con tres en línea

        else if (opponentCount == 2 && emptyCount == 2) { score -= 50; }// Oponente con dos en línea
